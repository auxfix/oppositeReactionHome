...
export class PlyrComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() private plyrType: Plyr.MediaType = 'audio';
  ...
  private initPlyr(force = false) {
    if (force || !this.player) {
      this.ngZone.runOutsideAngular(() => {
        ...
        this.driver =  new DefaultPlyrDriver(); //this.plyrDriver ||
  ...
  private ensureAudioElement() {
    const audioElement = this.hostElement.querySelector('audio');
    ...
    if (audioElement) {
      ...
      this.audioElement.setAttribute('crossorigin', '');
      this.audioElement.setAttribute('playsinline', '');
      this.renderer.appendChild(this.hostElement, this.audioElement);
    }
  }
